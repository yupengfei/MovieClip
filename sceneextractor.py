...
# ffprobe -i zhaowen.mp4 -show_format 2>&1 | grep 'duration='
# 返回值为 duration=3058.320000
...
# ffprobe -show_frames -of compact=p=0 -f lavfi "movie=zhaowen.mp4,select=gt(scene\,.4)" > scenes.txt
...
extract_command = "ffmpeg -i %s -f segment -segment_times %s -c:v copy -c:a copy %s/%%04d.mp4 2>&1"
